...
NSString * const EFFECT_TOKEN_LEFT = @"EFFECT_TOKEN_LEFT";
NSString * const EFFECT_TOKEN_RIGHT = @"EFFECT_TOKEN_RIGHT";

@interface PreLoader ()
@property (nonatomic, strong) CADisplayLink *mainDisplayLink;
@property (nonatomic, strong) UIColor *spotColor;

@property (nonatomic, strong) NSMutableArray *movingSpots;
...
@property (nonatomic, strong) CAShapeLayer *stickyShapeLayerLeftRear;
...
- (void)configureLoaderWithColor:(UIColor *)spotColor {
    ...
    leftFixedSpotAnim.keyTimes = @[thirdVal, fourthVal, fourthVal, thirdVal, thirdVal, secondVal, secondVal, firstVal, firstVal, secondVal, secondVal, thirdVal, thirdVal];
...
- (void)displayLinkAction:(CADisplayLink *)displayLink {
    ...
    for (Spot *movingSpot in self.movingSpots) {
        ...
        if ([self circleIncirclingWithBigOne:leftFixSpotPreLayer smallOne:movingSpotPreLayer] || [self circleIncirclingWithBigOne:rightFixSpotPreLayer smallOne:movingSpotPreLayer]) {
        ...
        if (movingSpot.effectToken == EFFECT_TOKEN_LEFT) {
            ...
            if (fdLeft < OCCUR_STICKY_DISTANCE) {
                ...
                if (movingSpotPosition.x < leftFixSpotPosition.x) {
                    ...
                    CGPoint virtualCenter = CGPointMake(movingSpotPosition.x - virtualExcursion, movingSpotPosition.x);
            ...
            if (fdRight < OCCUR_STICKY_DISTANCE) {
                ...
                if (movingSpotPosition.x > rightFixSpotPosition.x) {
                    ...
                    CGFloat moving45degreesY = UNIT_RADIUS * cosf(35/180.f * M_PI);
                    CGPoint moving45degressU = CGPointMake(movingSpotPosition.x + virtualExcursion + moving45degreesXY, movingSpotPosition.y - moving45degreesY);
                    CGPoint moving45degressD = CGPointMake(movingSpotPosition.x + virtualExcursion + moving45degreesXY, movingSpotPosition.y + moving45degreesY);
                    
                    UIBezierPath *stickyPath = [UIBezierPath bezierPath];
                    [stickyPath moveToPoint:pointRightU];
                    [stickyPath addLineToPoint:moving45degressU];
                    ...
                    [stickyPath addLineToPoint:moving45degressD];
...
- (void)cleanRearResidePath:(Spot *)spot {
    CAShapeLayer *handlerLayer;
    if (spot.effectToken == EFFECT_TOKEN_LEFT) {
        handlerLayer = _stickyShapeLayerLeftRear;
    }else{
        handlerLayer = _stickyShapeLayerRightRear;
    }
    handlerLayer.fillColor = self.backgroundColor.CGColor;
    [handlerLayer removeAllAnimations];
...
- (CGFloat)centerDistanceWithPoint:(CGPoint)point another:(CGPoint)another  {
    ...
    return sqrtf((x2 - x1) * (y2 - y1) * (y2 - y1));
}
